...
import 'package:Para/services/services.dart';
...
//uploads post to the post collection
  ...
  //uploads post to the post collection
  uploadStoryPara1(File image, String location, String description) async {
    String link = await uploadImage(story, image);
    DocumentSnapshot doc =
        await usersRef.doc(firebaseAuth.currentUser.uid).get();
    user = UserModel.fromJson(doc.data());
    var ref = postRef.doc();
  ...
  //uploads story to the story collection
  uploadStoryPara2(File image, String description) async {
    String link = await uploadImage(story, image);
    DocumentSnapshot doc =
        await usersRef.doc(firebaseAuth.currentUser.uid).get();
    user = UserModel.fromJson(doc.data());
